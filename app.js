...
function loadDayData(dayId,date){return lsGet(dataKey(dayId,date||today()))||{exercises:{},warmups:{},rpe:{}}}
function saveDayData(dayId,data){lsSet(dataKey(dayId,today()),data)}
function getHistory(dayId,exId,limit){
  var results=[],prefix=LS+dayId+"@";
  for(var i=0;i<localStorage.length;i++){var k=localStorage.key(i);if(k&&k.startsWith(prefix)){var date=k.slice(prefix.length);if(date===today())continue;try{var data=JSON.parse(localStorage.getItem(k));if(data&&data.exercises&&data.exercises[exId])results.push({date:date,sets:data.exercises[exId]})}catch(e){}}}
  results.sort(function(a,b){return b.date.localeCompare(a.date)});return results.slice(0,limit||5);
}
...
function getOverloadSuggestion(dayId,exercise){
  ...
  var allTop=comp.every(function(s){return parseInt(s.reps)>=range.max});if(!allTop)return null;
  var lw=parseFloat(comp[0].weight)||0;if(lw===0)return null;var inc=exercise.increment||(exercise.machine?5:5);
  return{from:lw,to:lw+inc,increment:inc};
}
...
function RestTimer(props){
  var exKey=props.exKey,defaultSeconds=props.defaultSeconds,startTrigger=props.startTrigger||0;
  var timers=useTimers();
  var s=useState(function(){var t=timers.getTimer(exKey);return t?t.total:defaultSeconds}),total=s[0],setTotal=s[1];
  var s2=useState(function(){var t=timers.getTimer(exKey);if(t&&t.running)return Math.max(0,t.total-Math.floor((Date.now()-t.startedAt)/1000));if(t&&t.done)return 0;return null}),remaining=s2[0],setRemaining=s2[1];
  var s3=useState(function(){var t=timers.getTimer(exKey);return t?t.running:false}),running=s3[0],setRunning=s3[1];
  var s4=useState(function(){var t=timers.getTimer(exKey);return t?!!t.done:false}),done=s4[0],setDone=s4[1];
  var intervalRef=useRef(null),totalRef=useRef(total);
  var clearInt=useCallback(function(){if(intervalRef.current){clearInterval(intervalRef.current);intervalRef.current=null}},[]);
  var startFn=useCallback(function(){
    clearInt();timers.triggerTimer(exKey,totalRef.current);
    var now=Date.now();
    setDone(false);setRemaining(totalRef.current);setRunning(true);
    intervalRef.current=setInterval(function(){var left=Math.max(0,totalRef.current-Math.floor((Date.now()-now)/1000));setRemaining(left);if(left===0){clearInterval(intervalRef.current);intervalRef.current=null;setRunning(false);setDone(true);var t=timers.getTimer(exKey);if(t){t.running=false;t.done=true;timers.setTimer(exKey,t)}if(navigator.vibrate)navigator.vibrate([200,100,200])}},250);
  },[exKey,clearInt,timers]);
  var stopFn=useCallback(function(){clearInt();setRunning(false);setRemaining(null);setDone(false);timers.setTimer(exKey,{total:totalRef.current,startedAt:null,running:false,done:false})},[exKey,clearInt,timers]);
  useEffect(function(){var t=timers.getTimer(exKey);if(t&&t.running){setRunning(true);setDone(false);setTotal(t.total);totalRef.current=t.total;var st=t.startedAt;intervalRef.current=setInterval(function(){var left=Math.max(0,t.total-Math.floor((Date.now()-st)/1000));setRemaining(left);if(left===0){clearInterval(intervalRef.current);intervalRef.current=null;setRunning(false);setDone(true);t.running=false;t.done=true;timers.setTimer(exKey,t);if(navigator.vibrate)navigator.vibrate([200,100,200])}},250)}return function(){clearInt()}},[exKey,clearInt]);
  useEffect(function(){totalRef.current=total},[total]);
  useEffect(function(){if(startTrigger>0){var t=timers.getTimer(exKey);if(t&&t.running&&t.startedAt){setTotal(t.total);totalRef.current=t.total;setDone(false);setRunning(true);var st=t.startedAt;clearInt();setRemaining(Math.max(0,t.total-Math.floor((Date.now()-st)/1000)));intervalRef.current=setInterval(function(){var left=Math.max(0,t.total-Math.floor((Date.now()-st)/1000));setRemaining(left);if(left===0){clearInterval(intervalRef.current);intervalRef.current=null;setRunning(false);setDone(true);t.running=false;t.done=true;timers.setTimer(exKey,t);if(navigator.vibrate)navigator.vibrate([200,100,200])}},250)}}},[startTrigger]);
  ...
  return h("div",{style:{display:"flex",alignItems:"center",gap:10,padding:"8px 0"}},
    ...
    h("div",{style:{display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}},
      ...
      h("div",{style:{display:"flex",gap:3}},presets.map(function(sec){return h("button",{key:sec,onClick:function(){setTotal(sec);totalRef.current=sec;if(!running){setRemaining(null);setDone(false);timers.setTimer(exKey,{total:sec,startedAt:null,running:false,done:false})}},style:{padding:"3px 6px",borderRadius:5,fontSize:10,fontWeight:600,cursor:"pointer",background:total===sec?"rgba(245,158,11,0.18)":"rgba(255,255,255,0.04)",color:total===sec?"#f59e0b":"#6b7280",border:total===sec?"1px solid rgba(245,158,11,0.3)":"1px solid rgba(255,255,255,0.07)"}},sec<60?sec+"s":(sec/60|0)+(sec%60?":"+(sec%60+"").padStart(2,"0"):"m"))}))));
}

/* ── History Panel ── */
function HistoryPanel(props){
  var hist=useMemo(function(){return getHistory(props.dayId,props.exId,5)},[props.dayId,props.exId]);
  if(!hist.length)return h("div",{style:{fontSize:11,color:"#3f3f46",fontStyle:"italic",padding:"6px 0"}},"No previous sessions yet.");
  return h("div",{className:"fade-in",style:{marginTop:6}},hist.map(function(entry){var d=new Date(entry.date+"T12:00:00");var label=d.toLocaleDateString("en-US",{month:"short",day:"numeric"});return h("div",{key:entry.date,style:{display:"flex",alignItems:"center",gap:8,padding:"4px 0",borderBottom:"1px solid rgba(255,255,255,0.03)"}},h("span",{style:{fontSize:10,fontWeight:700,color:"#4b5563",width:44,flexShrink:0}},label),h("div",{style:{display:"flex",gap:6,flex:1,flexWrap:"wrap"}},entry.sets.map(function(s,i){return(s.weight||s.reps)?h("span",{key:i,style:{fontSize:10,color:s.done?"#6b7280":"#4b5563",fontVariantNumeric:"tabular-nums",whiteSpace:"nowrap"}},(s.weight||"—")+"×"+(s.reps||"—")):null}).filter(Boolean)))}));
}
...
function ExerciseCard(props){
  var exercise=props.exercise,index=props.index,dayId=props.dayId,onSetUpdate=props.onSetUpdate;
  ...
  var str=useState(0),timerTrig=str[0],bumpTimerTrig=str[1];
  var exKey=dayId+"_"+exercise.id;var saved=dayData.getData(dayId);var exData=saved.exercises&&saved.exercises[exercise.id];
  var completedSets=exData?exData.filter(function(s){return s.done}).length:0;var allDone=completedSets===exercise.sets;
  var timerData=timers.getTimer(exKey);
  var timerRunning=timerData&&timerData.running;
  var overload=useMemo(function(){return getOverloadSuggestion(dayId,exercise)},[dayId,exercise]);
  ...
  var onToggleDone=useCallback(function(){
    if(getAutoTimer()){timers.triggerTimer(exKey,exercise.rest);bumpTimerTrig(function(t){return t+1})}
  ...
  return h("div",{style:{background:allDone?"rgba(34,197,94,0.03)":"rgba(255,255,255,0.025)",borderRadius:14,border:allDone?"1px solid rgba(34,197,94,0.12)":"1px solid rgba(255,255,255,0.055)",padding:"12px 14px",marginBottom:8,transition:"all 0.2s"}},
    h("div",{onClick:function(){setExpanded(!expanded)},style:{cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8,WebkitUserSelect:"none",userSelect:"none"}},
      h("div",{style:{flex:1}},h("div",{style:{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}},h("span",{style:{fontSize:10,fontWeight:800,color:allDone?"#22c55e":"#f59e0b",background:allDone?"rgba(34,197,94,0.1)":"rgba(245,158,11,0.1)",padding:"2px 6px",borderRadius:4,letterSpacing:.4,lineHeight:"16px"}},index+1),h("span",{style:{fontSize:14,fontWeight:700,color:allDone?"#6b7280":"#f1f5f9",lineHeight:1.3,textDecoration:allDone?"line-through":"none",textDecorationColor:"rgba(107,114,128,0.4)"}},exercise.name),overload&&!allDone?h("span",{style:{fontSize:10,fontWeight:700,color:"#22c55e",background:"rgba(34,197,94,0.1)",padding:"2px 6px",borderRadius:4,whiteSpace:"nowrap"}},"↑ "+overload.to+" "+unit):null),h("div",{style:{display:"flex",alignItems:"center",gap:6,marginTop:3}},h("span",{style:{fontSize:11,color:"#6b7280"}},exercise.sets+"×"+exercise.reps+" · "+restLabel),completedSets>0?h("span",{style:{fontSize:10,fontWeight:700,color:allDone?"#22c55e":"#f59e0b",background:allDone?"rgba(34,197,94,0.1)":"rgba(245,158,11,0.08)",padding:"1px 5px",borderRadius:4}},completedSets+"/"+exercise.sets):null,timerRunning&&!expanded?h("span",{className:"timer-active",style:{width:6,height:6,borderRadius:3,background:"#f59e0b"}}):null),!expanded?h("div",{style:{fontSize:11,color:"#4b5563",marginTop:2,fontStyle:"italic"}},exercise.notes):null),
      h("span",{style:{fontSize:16,color:"#3f3f46",transform:expanded?"rotate(180deg)":"rotate(0deg)",transition:"transform 0.2s",marginTop:2,flexShrink:0}},"\u25BE")),
...
function BodyMetrics(props){
  ...
  var s=useState(function(){return lsGet(metricsKey)||{bodyweight:"",waist:"",notes:""}}),data=s[0],setData=s[1];
  ...
  var wLabel=unit==="kg"?"BODYWEIGHT (kg)":"BODYWEIGHT (lbs)";var mLabel=unit==="kg"?"WAIST (cm)":"WAIST (inches)";
  return h("div",{className:"overlay",onClick:function(e){if(e.target===e.currentTarget)onClose()}},h("div",{className:"sheet fade-in"},h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},h("h3",{style:{fontSize:18,fontWeight:800,color:"#f1f5f9"}},"Body Metrics"),h("button",{onClick:onClose,style:{background:"none",border:"none",color:"#6b7280",fontSize:20,cursor:"pointer"}},"✕")),h("div",{style:{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}},h("div",null,h("label",{style:{fontSize:11,fontWeight:700,color:"#6b7280",display:"block",marginBottom:6}},wLabel),h("input",{type:"number",inputMode:"decimal",value:data.bodyweight,onChange:function(e){update("bodyweight",e.target.value)},placeholder:unit==="kg"?"63":"140",style:iStyle})),h("div",null,h("label",{style:{fontSize:11,fontWeight:700,color:"#6b7280",display:"block",marginBottom:6}},mLabel),h("input",{type:"number",inputMode:"decimal",value:data.waist,onChange:function(e){update("waist",e.target.value)},placeholder:unit==="kg"?"81":"32",style:iStyle}))),h("div",null,h("label",{style:{fontSize:11,fontWeight:700,color:"#6b7280",display:"block",marginBottom:6}},"NOTES"),h("input",{type:"text",value:data.notes||"",onChange:function(e){update("notes",e.target.value)},placeholder:"Sleep, energy, etc.",style:Object.assign({},iStyle,{textAlign:"left",fontSize:13})})),history.length>0?h("div",{style:{marginTop:16}},h("div",{style:{fontSize:12,fontWeight:700,color:"#6b7280",marginBottom:8}},"Recent Entries"),h("div",{style:{maxHeight:200,overflowY:"auto"}},history.map(function(entry){var d=new Date(entry.date+"T12:00:00");var label=d.toLocaleDateString("en-US",{month:"short",day:"numeric"});return h("div",{key:entry.date,style:{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,0.03)",fontSize:12}},h("span",{style:{color:"#6b7280",fontWeight:600}},label),h("div",{style:{display:"flex",gap:12}},entry.bw?h("span",{style:{color:"#e5e7eb"}},entry.bw+" "+unit):null,entry.waist?h("span",{style:{color:"#9ca3af"}},entry.waist+(unit==="kg"?" cm":'"')):null))}))):null));
}
...
function DayView(props){
  ...
  return h("div",{style:{paddingBottom:40}},h("div",{style:{marginBottom:14}},h("h2",{style:{fontSize:18,fontWeight:800,margin:0,color:"#f1f5f9",letterSpacing:-.3}},day.title),h("p",{style:{fontSize:12,color:"#6b7280",margin:"3px 0 0",fontStyle:"italic"}},day.subtitle),h("div",{style:{marginTop:10,display:"flex",alignItems:"center",gap:10}},h("div",{style:{flex:1,height:4,borderRadius:2,background:"rgba(255,255,255,0.06)",overflow:"hidden"}},h("div",{style:{width:(pct*100)+"%",height:"100%",borderRadius:2,background:pct>=1?"#22c55e":"#f59e0b",transition:"width 0.3s"}})),h("span",{style:{fontSize:11,fontWeight:700,color:pct>=1?"#22c55e":"#9ca3af",fontVariantNumeric:"tabular-nums",flexShrink:0}},doneSets+"/"+totalSets),allComplete?h("button",{onClick:function(){setShowComplete(true)},style:{fontSize:10,fontWeight:700,color:"#22c55e",background:"rgba(34,197,94,0.1)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:6,padding:"3px 8px",cursor:"pointer"}},"Summary"):null)),
    day.exercises.map(function(ex,i){return h(ExerciseCard,{key:ex.id,exercise:ex,index:i,dayId:day.id,onSetUpdate:refresh})}),
    customs.length>0?h("div",{style:{borderTop:"1px solid rgba(99,102,241,0.1)",marginTop:8,paddingTop:8}},h("div",{style:{fontSize:10,fontWeight:700,color:"#818cf8",marginBottom:6,letterSpacing:.5}},"CUSTOM EXERCISES"),customs.map(function(ex,i){return h("div",{key:ex.id,style:{position:"relative"}},h(ExerciseCard,{exercise:ex,index:day.exercises.length+i,dayId:day.id,onSetUpdate:refresh}),h("button",{onClick:function(){removeCustom(ex.id)},style:{position:"absolute",top:10,right:10,width:22,height:22,borderRadius:6,border:"1px solid rgba(239,68,68,0.3)",background:"rgba(239,68,68,0.08)",color:"#ef4444",fontSize:10,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",zIndex:10}},"✕"))})):null,
...
function SessionTimer(){var s=useState(0),elapsed=s[0],setElapsed=s[1];var started=getSessionStart();useEffect(function(){if(!started)return;var tick=function(){setElapsed(Math.floor((Date.now()-started)/1000))};tick();var id=setInterval(tick,1000);return function(){clearInterval(id)}},[started]);if(!started)return h("span",{style:{fontSize:11,color:"#3f3f46"}},"Not started");return h("span",{style:{fontSize:12,fontWeight:700,color:"#f59e0b",fontVariantNumeric:"tabular-nums"}},fmtElapsed(elapsed))}
  ...
  return h("div",{style:{display:"flex",flexDirection:"column",height:"100vh",overflow:"hidden"}},
    h("div",{style:{paddingTop:"max(env(safe-area-inset-top, 0px), 12px)",paddingLeft:16,paddingRight:16,paddingBottom:10,borderBottom:"1px solid rgba(255,255,255,0.06)",background:"rgba(10,10,15,0.97)",flexShrink:0,backdropFilter:"blur(20px)",WebkitBackdropFilter:"blur(20px)"}},
      ...
      h("div",{style:{display:"flex",gap:3}},DAYS.map(function(day,i){var saved=dayData.getData(day.id);var customs=getCustomExercises(day.id);var allEx=day.exercises.concat(customs);var doneSets=allEx.reduce(function(a,e){var d=saved.exercises&&saved.exercises[e.id];return a+(d?d.filter(function(s){return s.done}).length:0)},0);var totalSets=allEx.reduce(function(a,e){return a+e.sets},0);var hasProg=doneSets>0,complete=doneSets===totalSets&&totalSets>0;
        return h("button",{key:day.id,onClick:function(){setActiveDay(i)},style:{flex:1,padding:"7px 2px 5px",borderRadius:9,border:activeDay===i?"1px solid rgba(245,158,11,0.4)":complete?"1px solid rgba(34,197,94,0.15)":"1px solid rgba(255,255,255,0.04)",background:activeDay===i?"rgba(245,158,11,0.1)":complete?"rgba(34,197,94,0.04)":"rgba(255,255,255,0.02)",cursor:"pointer",textAlign:"center",transition:"all 0.15s"}},h("div",{style:{fontSize:9,fontWeight:700,color:activeDay===i?"#f59e0b":complete?"#22c55e":"#4b5563",letterSpacing:.4}},dayMap[day.id]),h("div",{style:{fontSize:11,fontWeight:700,color:activeDay===i?"#f1f5f9":complete?"#6b7280":"#6b7280",marginTop:1}},day.label),(hasProg||complete)?h("div",{style:{width:4,height:4,borderRadius:2,background:complete?"#22c55e":"#f59e0b",margin:"3px auto 0"}}):null)}))),
    /* Content */
    h("div",{ref:scrollRef,style:{flex:1,overflowY:"auto",WebkitOverflowScrolling:"touch",paddingTop:16,paddingLeft:16,paddingRight:16,paddingBottom:"max(env(safe-area-inset-bottom, 0px), 20px)"}},h(DayView,{key:activeDay,day:DAYS[activeDay],refresh:refresh,tick:tick})),
    /* Floating timer */
