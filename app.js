...
function initProfile(p){PROFILE=p;LS="ht_"+p+"_";
  /* Clear history index on profile change (prevents cross-profile contamination) */
  _historyIndex={};_historyBuilt=false;
  /* Restore session date lock in case of page reload during a midnight workout */
  try{var lock=localStorage.getItem(LS+"session_lock");if(lock){_sessionDateLock=lock;if(!getSessionStart())unlockSessionDate()}}catch(e){}
...
function lockSessionDate(){if(!_sessionDateLock){_sessionDateLock=today();try{localStorage.setItem(LS+"session_lock",_sessionDateLock)}catch(e){}}}
function unlockSessionDate(){_sessionDateLock=null;try{localStorage.removeItem(LS+"session_lock")}catch(e){}}
...
function FloatingTimer(){
  ...
  return h("div",{style:{position:"fixed",bottom:"calc(40px + env(safe-area-inset-bottom,0px))",left:0,right:0,zIndex:100,padding:"10px 16px",background:isDone?"rgba(34,197,94,0.15)":"rgba(10,10,15,0.95)",borderTop:isDone?"1px solid var(--success-border)":"1px solid var(--accent-border)",backdropFilter:"blur(12px)",WebkitBackdropFilter:"blur(12px)",display:"flex",alignItems:"center",justifyContent:"space-between",cursor:isDone?"pointer":"default"},onClick:isDone?function(){timers.setTimer(display.key,null)}:undefined,"aria-label":isDone?"Rest complete — tap to dismiss":"Rest timer: "+fmtTime(display.remaining)+" remaining"},
...
function HistoryPanel(props){
  ...
  var rpeData=useMemo(function(){return hist.map(function(entry){var dd=getIndexDataForDate(props.dayId,entry.date)||{};return dd.rpe&&dd.rpe[props.exId]?dd.rpe[props.exId]:null}).filter(function(v){return v!==null}).reverse()},[hist,props.dayId,props.exId]);
  /* Rest time analysis across sessions */
  var restAnalysis=useMemo(function(){
    var rests=[];
    hist.forEach(function(entry){
      var dd=getIndexDataForDate(props.dayId,entry.date)||{};
  ...
  return h("div",{className:"fade-in",style:{marginTop:6},"aria-label":"Exercise history"},
    ...
    hist.map(function(entry){var label=formatDate(entry.date);var bestE1rm=0;entry.sets.forEach(function(s){if(s.done&&s.weight&&s.reps){var e=calc1RM(s.weight,s.reps);if(e>bestE1rm)bestE1rm=e}});var pastData=getIndexDataForDate(props.dayId,entry.date)||{};var rpe=pastData.rpe&&pastData.rpe[props.exId]?pastData.rpe[props.exId]:null;var rpeColor=rpe?rpe<=7?"var(--success)":rpe===8?"var(--accent)":rpe===9?"var(--warning)":"var(--danger)":null;var exNote=pastData.exNotes&&pastData.exNotes[props.exId]?pastData.exNotes[props.exId]:null;
...
function SetLogger(props){
  ...
  return h("div",{style:{marginTop:4},"aria-label":"Set logger"},
    ...
    data.map(function(set,i){var ghost=lastSession&&lastSession[i];var ghostWeight=ghost&&ghost.weight?intensityMult?String(unit==="kg"?Math.round(parseFloat(ghost.weight)*intensityMult*2)/2:Math.round(parseFloat(ghost.weight)*intensityMult/5)*5):String(ghost.weight):null;var isExtra=i>=numSets;var isNextSet=i===nextSetIdx;
...
/* ── Close Button (shared modal/sheet close button) ── */
function CloseBtn(props){
  return h("button",{onClick:props.onClick,"aria-label":"Close",style:{background:"none",border:"none",color:"var(--text-dim)",fontSize:20,cursor:"pointer"}},"\u2715");
}

/* ── RPE Button Group (shared by RPERating and SessionRPE) ── */
function RPEButtonGroup(props){
  var value=props.value,onSelect=props.onSelect,labels=props.labels,groupLabel=props.groupLabel||"Rate perceived exertion";
  return h("div",{style:{display:"flex",gap:4},role:"radiogroup","aria-label":groupLabel},[6,7,8,9,10].map(function(val){var active=value===val;
    return h("button",{key:val,onClick:function(){onSelect(val)},className:"rpe-btn",style:{border:active?"2px solid "+RPE_COLORS[val]:"1px solid rgba(255,255,255,0.08)",background:active?"rgba(255,255,255,0.06)":"transparent"},role:"radio","aria-checked":active?"true":"false","aria-label":"RPE "+val+" - "+labels[val]},
      h("div",{style:{fontSize:15,fontWeight:800,color:active?RPE_COLORS[val]:"var(--text-dim)"}},val),
...
function RPERating(props){
  ...
  return h("div",{className:"fade-in",style:{marginTop:8,padding:"8px 0"},"aria-label":"RPE rating"},
    ...
    h(RPEButtonGroup,{value:rpe,onSelect:save,labels:labels}),
...
function ExerciseOverrideEditor(props){
  ...
  return h("div",{style:{marginTop:4}},
    ...
    showEdit&&h("div",{className:"fade-in",style:{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginTop:6}},
      ...
      h("div",null,h("label",{style:{fontSize:9,fontWeight:700,color:"var(--text-dim)"}},"REPS"),h("input",{type:"text",value:form.reps,placeholder:exercise.reps,onChange:function(e){upd("reps",e.target.value)},className:"input",style:{fontSize:12},"aria-label":"Reps"})),
      h("div",null,h("label",{style:{fontSize:9,fontWeight:700,color:"var(--text-dim)"}},"REST (s)"),h("input",{type:"number",inputMode:"numeric",value:form.rest,placeholder:String(exercise.rest),onChange:function(e){upd("rest",e.target.value)},className:"input",style:{fontSize:12},"aria-label":"Rest in seconds"})),
...
function ExerciseCard(props){
  ...
  var meso=props.meso||getMesocycle();var isDeloadWeek=meso.week===4;
  var mesoRepTarget=meso.mode==="undulating"?getMesoRepTarget(exercise.reps,meso.week):null;
  var overload=useMemo(function(){if(isDeloadWeek)return null;return getOverloadSuggestion(dayId,exercise)},[dayId,exercise.id,exercise.sets,exercise.reps,exercise.increment,exercise.bilateral,isDeloadWeek]);
  var deloadMod=useMemo(function(){if(!isDeloadWeek)return null;return getDeloadModifiers(dayId,exercise)},[dayId,exercise.id,isDeloadWeek]);
  ...
  return h("div",{className:cardClass,style:{position:"relative"},"aria-label":exercise.name},
    ...
    showSwapMenu?h("div",{className:"fade-in",style:{padding:"6px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}},
      ...
      h("div",{style:{marginTop:4}},
        h("input",{type:"text",value:swapSearch,onChange:function(e){setSwapSearch(e.target.value)},placeholder:"Search all exercises\u2026","aria-label":"Search exercises to swap",style:{width:"100%",padding:"6px 8px",background:"var(--surface-alt)",border:"1px solid var(--border)",borderRadius:6,color:"var(--text-primary)",fontSize:16},onClick:function(e){e.stopPropagation()}}),
...
function VolumeDashboard(props){
  ...
  var prevVol=useMemo(function(){return calcPreviousWeekVolume(config)},[config]);
  ...
  return h(Overlay,{onClose:onClose,label:"Weekly Volume"},
    h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
      h("h3",{style:{fontSize:18,fontWeight:800,color:"var(--text-bright)"}},"Weekly Volume"),
      h("div",{style:{display:"flex",gap:8}},h("button",{onClick:function(){setEditing(!editing)},className:"btn btn--ghost btn--xs"},editing?"Cancel":"Targets"),h(CloseBtn,{onClick:onClose}))),
...
function getDeloadWarning(config){
  ...
  config.days.forEach(function(day){day.exercises.forEach(function(ex){
    ...
    for(var i=0;i<entries.length&&count<3;i++){
      var e=entries[i];if(e.date===td)continue;
      /* Only count entries where this exercise was performed */
      var hasData=ex.bilateral?(e.data.exercises&&(e.data.exercises[ex.id+"_L"]||e.data.exercises[ex.id+"_R"])):(e.data.exercises&&e.data.exercises[ex.id]);
      if(!hasData)continue;
      count++;
...
function BodyMetrics(props){
  ...
  return h(Overlay,{onClose:onClose,label:"Body Metrics"},
    h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},h("h3",{style:{fontSize:18,fontWeight:800,color:"var(--text-bright)"}},"Body Metrics"),h(CloseBtn,{onClick:onClose})),
    ...
    h("div",{style:{marginBottom:12}},h("label",{style:{fontSize:11,fontWeight:700,color:"var(--text-dim)",display:"block",marginBottom:6}},"NOTES"),h("input",{type:"text",value:data.notes||"",onChange:function(e){update("notes",e.target.value)},placeholder:"Sleep, energy, etc.",className:"input input--text input--lg","aria-label":"Daily notes"})),
...
function SessionRPE(props){
  ...
  var labels={6:"Easy",7:"Moderate",8:"Hard",9:"Very Hard",10:"Max"};
  return h("div",{className:"fade-in",style:{background:"var(--success-bg)",border:"1px solid var(--success-border)",borderRadius:12,padding:"14px 16px",marginTop:12},"aria-label":"Session RPE rating"},
    ...
    h(RPEButtonGroup,{value:rpe,onSelect:save,labels:labels,groupLabel:"Session RPE"}),
...
function SessionTimer(props){
  ...
  return h("div",{style:{position:"relative",display:"inline-flex",alignItems:"center"}},
    h("button",{onClick:function(){setShowMenu(!showMenu)},style:{fontSize:12,fontWeight:700,color:"var(--accent)",fontVariantNumeric:"tabular-nums",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:3,background:"none",border:"none",padding:0,minHeight:44},"aria-label":"Session duration: "+fmtElapsed(elapsed)+". Click to manage session","aria-expanded":showMenu?"true":"false","aria-haspopup":"true"},fmtElapsed(elapsed),h("span",{style:{fontSize:8,color:"var(--text-dim)"},"aria-hidden":"true"},"\u25BE")),
...
function StandaloneCardio(props){
  var onClose=props.onClose;
  return h(Overlay,{onClose:onClose,label:"Rest Day Cardio"},
    h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
...
function SettingsPanel(props){
  ...
  return h(Overlay,{onClose:onClose,label:"Settings"},
    ...
    showMore?h("div",{className:"overlay",onClick:function(e){if(e.target===e.currentTarget)setShowMore(false)},role:"dialog","aria-modal":"true","aria-label":"More options"},
      h("div",{className:"sheet fade-in",ref:moreRef,style:{paddingBottom:"max(env(safe-area-inset-bottom,0px),24px)"}},
        h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
          ...
          h(CloseBtn,{onClick:function(){setShowMore(false)}})),
...
function TemplateManager(props){
  ...
  return h(Overlay,{onClose:onClose,label:"Workout Templates"},
    h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
...
function FatigueTrendChart(props){
  ...
  if(scored.length<2)return h(Overlay,{onClose:onClose,label:"Fatigue Trend"},
    h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
  ...
  return h(Overlay,{onClose:onClose,label:"Fatigue Trend"},
    h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
      h("h3",{style:{fontSize:18,fontWeight:800,color:"var(--text-bright)"}},"Fatigue Trend"),
      h(CloseBtn,{onClick:onClose})),
...
function WorkoutCalendar(props){
  ...
  var todayStr=today();
  return h(Overlay,{onClose:onClose,label:"Workout Calendar"},
    h("div",{style:{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}},
